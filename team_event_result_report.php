...
if(isset($_POST['submit']))
{
	...
	echo "<script>window.location='team_event_result.php?event_id=$_GET[event_id]';</script>";
...
<table class="table table-bordered" style="color: white">
	...
	<tr>
		...
		<td style="text-align: left;"><div class="row">
		  ...
		  <br>
          <div class="login_form" style="padding-top: 10px;padding-bottom: 10px;">
		  
		          <div class="col-md-12">
          <centeR><div class="detail-box">
            <h5 style="color: yellow;">
              <b>Participant Result</b>
            </h5>
          </div></center>
        </div>	
		</div>
	...
	<?php
	$count = 1;
  	$sqlview = "SELECT event_result_status.*,student.student_name, student.student_rollno, student.st_class, student.student_image FROM event_result_status LEFT JOIN student ON student.student_id=event_result_status.student_id WHERE event_result_status.event_id='$_GET[event_id]' AND event_result_status.team='Team Leader' AND  event_result_status.winning_position IN (1,2,3)  ORDER BY winning_position";
	$qsqlview = mysqli_query($con,$sqlview);
	echo mysqli_error($con);
	...
	<table class="table table-bordered" style="color: white;">
		...
		<tr>
			...
			<th>Ranking : <?php echo $rsview['winning_position']; ?></th>
			<th>Point: <?php echo $rsview['point']; ?></th>
		</tr>
		</thead>
	</table>
	<table class="table table-bordered" style="color: white">
